  ...
  return (
    <div className="grid grid-cols-2 gap-3.5 md:grid-cols-3 lg:grid-cols-5">
      <div className="rounded-2xl bg-brand p-5 shadow-[0_6px_18px_rgba(200,19,27,.22)]">
        <p className={`${LABEL} text-[#F6C9C7]`}>Total em risco</p>
        <p className={`${VALUE} text-white`}>{kpis.totalEmRisco.toLocaleString('pt-BR')}</p>
        <p className="mt-2 text-xs text-[#F2B4B1]">crítico + alerta + nunca mov.</p>
      </div>
