...
interface Props {
  ...
  icon: LucideIcon;
  hideFilters?: boolean;
  /** Exibe o botão "Atualizar dados" (webhook do Sucesso) no cabeçalho. */
  showRefresh?: boolean;
}

export const SucessoPlaceholder = ({ title, description, icon: Icon, hideFilters, showRefresh }: Props) => {
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-[1400px] space-y-4 px-4 py-6 sm:px-6 md:px-10">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          {showRefresh && <RefreshDataButton event="atualizar_dados_sucesso" invalidateKey="dash_sucesso" />}
        </div>
