...
export const MrrAtivadoMesModal = ({
  ...
  const porAgente = useMemo(() => {
    ...
    return [...map.entries()]
      .map(([nome, v]) => ({ nome, ...v, pct: totalMrr > 0 ? (v.mrr / totalMrr) * 100 : 0 }))
  ...
  const toggleSort = (k: SortKey) => {
    if (sortKey === k) setSortDir(sortDir === "asc" ? "desc" : "asc");
      ...
      setSortKey(k);
      setSortDir(k === "cliente" || k === "agente" || k === "perfil" ? "asc" : "desc");
    }
    setPage(0);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[95vw] sm:w-auto max-w-5xl max-h-[90vh] overflow-y-auto rounded-2xl">
        <DialogHeader>
              ...
              {porAgente.length > 0 && (
                <TableBody>
                  <TableRow className="bg-muted/40 font-semibold">
                    ...
                    <TableCell className="text-right font-numeric tabular-nums">{totalQtd}</TableCell>
                    <TableCell className="text-right font-numeric tabular-nums">
                      {fmtBRL(totalMrr)}
                    </TableCell>
                    <TableCell className="text-right font-numeric tabular-nums">100,0%</TableCell>
                  </TableRow>
                </TableBody>
              )}
            </Table>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between gap-3">
            <h3 className="font-display text-sm font-semibold text-secondary">
              Deals ativados ({filtered.length.toLocaleString("pt-BR")})
            </h3>
