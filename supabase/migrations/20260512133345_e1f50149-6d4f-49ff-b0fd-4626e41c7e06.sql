...
CREATE OR REPLACE FUNCTION public.list_operators()
 RETURNS TABLE(user_id uuid, email text, full_name text, avatar_url text, role operations_role, agente_ativacao text, created_at timestamp with time zone, last_sign_in_at timestamp with time zone, email_confirmed_at timestamp with time zone, invited_at timestamp with time zone)
 LANGUAGE plpgsql
 SECURITY DEFINER
