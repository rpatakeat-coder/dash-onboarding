...
- **⚠️ Formato de data**: em `dash_sucesso`, `data_fechamento` e `data_entrada_fase` são **string BR `DD/MM/YYYY HH:MM:SS`**, NÃO ISO. `new Date(...)` retorna Invalid Date — use `parseDataFlex` (exportado de `useDashSucesso.ts`), que trata BR e cai para ISO. O filtro de período global (`applySucessoFilter` → `parseDateLoose` → `parseDataFlex`) e o `selectChurn` (churn-por-mês) **já usam** esse parser, então período ≠ "Tudo" funciona. Componentes que casam por string (`ChurnSucesso`, `Churn.tsx`, `SucessoClientesModal`, `UpsellSucesso`) seguem OK; ao escrever lógica nova de data, prefira `parseDataFlex`.
